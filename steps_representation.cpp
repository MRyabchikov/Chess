...
DangerSign::DangerSign(Point center, Chessboard& chess_) : Circle{center, c_size / 2}
{
    ...
    for (int i = center.x - c_size / 4 - c_size / 8; i <= center.x + c_size / 4 + c_size / 8;
         i += c_size / 4 + c_size / 8)
    {
    ...
    for (int j = center.y - c_size / 4 - c_size / 8; j <= center.y + c_size / 4 + c_size / 8;
         j += c_size / 2 + c_size / 4)
    {
...
RedCross::RedCross(Point center, Chessboard& chess_) : Rectangle{center, c_size, c_size}
{
    ...
    rectangle_1 = new Closed_polyline{{x - c_size / 2 + 2 * dist, y - c_size / 2 + dist},
    ...
    rectangle_2 = new Closed_polyline{{x + c_size / 2 - 2 * dist, y - c_size / 2 + dist},
                                      {x + c_size / 2 - dist, y - c_size / 2 + 2 * dist},
                                      {x - c_size / 2 + 2 * dist, y + c_size / 2 - dist},
                                      {x - c_size / 2 + dist, y + c_size / 2 - 2 * dist}};
    ...
    : Rectangle({center.x - c_size / 2, center.y - c_size / 2}, c_size, c_size)
{
    ...
    Rectangle* h_r4 =
        new Rectangle{{center.x + c_size / 2 - rc_width, center.y + c_size / 2 - rc_length}, rc_width, rc_length};
    ...
    Rectangle* v_r4 =
        new Rectangle{{center.x + c_size / 2 - rc_length, center.y + c_size / 2 - rc_width}, rc_length, rc_width};
